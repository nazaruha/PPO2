...
{
    public class StorageCreateValidation : AbstractValidator<StorageCreateDto>
    {
        public StorageCreateValidation()
        {
            RuleFor(s => s.ProjectId).NotEmpty();
            RuleFor(s => s.ProductId).NotEmpty();
            RuleFor(s => s.Price).NotEmpty();
            RuleFor(s => s.Count).NotEmpty();
            RuleFor(s => s.Description).NotEmpty().MaximumLength(maximumLength: 255);
